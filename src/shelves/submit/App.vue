  ...
  <div v-if="showApp">
    ...
    <weui-cells>
      <weui-cell label="名称">
        <input
          class="weui-input"
          @blur="setCache('product.name', product.name)"
          type="text"
          placeholder="请输入商品名(不超过25字)"
          maxlength="25"
      ...
      <weui-cell label="原价" v-if="isSpecial">
        <input
          class="weui-input"
          @blur="setCache('product.originPrice', product.originPrice)"
          v-model="product.originPrice"
          type="number"
          pattern="[0-9]*"
          @textInput="handlKeyDownPrice($event)"
          placeholder="请输入商品原价"
        >
      </weui-cell>
      <weui-cell>
        ...
        <input
          class="weui-input"
          @blur="setCache('product.sellPrice', product.sellPrice)"
          v-model="product.sellPrice"
          type="number"
          pattern="[0-9]*"
          @textInput="handlKeyDownPrice($event)"
          :placeholder="`请输入商品${isSpecial?'特价':'价格'}`"
        >
      </weui-cell>
      <weui-cell label="可售">
        <input
          class="weui-input"
          @blur="setCache('product.stock', product.stock)"
          type="number"
          pattern="[0-9]*"
          @textInput="handlKeyDownPrice($event)"
          placeholder="可售数量"
      ...
      <weui-cell label="规格">
        <input
          class="weui-input"
          @blur="setCache('product.spec', product.spec)"
          type="text"
          placeholder="斤/公斤/盒/包/套/件等"
          maxlength="10"
    ...
    <weui-cells>
      <weui-cell>
        <weui-textarea
          ...
          @blur="setCache('product.description', product.description)"
          maxlength="100"
          placeholder="请输入商品描述信息（可选）,不超过100字"
          rows="3"
        ></weui-textarea>
      </weui-cell>
    </weui-cells>
